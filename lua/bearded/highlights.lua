local c = require('bearded.colors')
local cfg = vim.g.bearded_config
local util = require("bearded.util")
...
hl.common = {
    ...
    MatchParen = {fg = c.none, bg = c.bg_d},
    NonText = {fg = c.grey},
    Whitespace = {fg = c.grey},
    SpecialKey = {fg = c.grey},
    Pmenu = {fg = c.fg, bg = c.bg1},
    PmenuSbar = {fg = c.none, bg = c.bg1},
    PmenuSel = {fg = c.bg0, bg = c.bg3},
    ...
    ToolbarButton = {fg = c.bg0, bg = c.bg3},
...
hl.syntax = {
    ...
    Keyword = {fg = c.dark_cyan, fmt = cfg.code_style.keywords},
...
if vim.api.nvim_call_function("has", { "nvim-0.8" }) == 1 then
    hl.treesitter = {
        ...
        ["@variable"] = {fg = c.pink, fmt = cfg.code_style.variables},
...
function M.setup()
        ...
        if color_name:sub(1, 1) == '$' then
            ...
            if not color_name then
                vim.schedule(function()
                    vim.notify('bearded.nvim: unknown color "' .. name .. '"', vim.log.levels.ERROR, { title = "bearded.nvim" })
                end)
                return ""
            end
        end
    ...
    for group_name, group_settings in pairs(vim.g.bearded_config.highlights) do
